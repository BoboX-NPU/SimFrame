...
- The project contains 20 unit and media tests plus 1 UI test covering frame scanning and replacement, device matching, PNG composition, isolated alpha-derived rounded screen masking, two-pixel bezel overlap, canvas behavior, bounded preview-area playback-control sizing, video orientation, codecs, audio, transparency, and target bitrate.
- Standard test command: `xcodebuild -project SimFrame.xcodeproj -scheme SimFrame -destination 'platform=macOS' test`
- On 2026-08-31, 18 reproducible unit and media tests passed with 0 failures. The two excluded local-fixture tests were the 30-frame Apple library scan and the high-resolution HEVC with Alpha audio fixture.
- The playback-preview tests passed with 0 failures, including explicit 400-point minimum, responsive intermediate, 1,000-point maximum width, and enlarged button constants. A running-app inspection confirmed that the control bar spans the available preview width, the enlarged buttons render correctly, Space alternates between playback and pause, playback advances its time and slider state, and the video remains inside the device-frame aperture.
