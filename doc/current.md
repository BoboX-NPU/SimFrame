...
- The repository includes ten flat 1024-by-1024 PNG app-icon concepts under `SimFrame/Resources/AppIconConcepts`. They are design candidates only and are not assigned to the application target.
...
- Shows a window-level material overlay immediately while a Device Frame library is imported or replaced. Directory scanning uses indeterminate progress, frame preparation reports a determinate processed/total count, and the overlay remains through atomic installation and first-frame artwork/mask loading. The existing preview stays visible beneath the overlay, video pauses without losing its position, and the workspace, inspector, toolbar, drag-and-drop surface, and Pickers remain disabled until the operation finishes.
- Allows Device Frame imports to be cancelled during scanning or per-frame preparation. Cancellation changes the overlay to a non-interactive cleanup state and removes staging data before restoring the workspace; atomic installation and first-frame loading cannot be cancelled, so a cancelled or failed replacement preserves the existing installed library without ambiguity.
...
- The project contains 36 unit and media tests plus 1 UI test. Coverage includes compact shared alpha analysis, an opt-in real 30-frame import benchmark, frame scanning and atomic replacement, import phase ordering and invalid-file progress, cancellation cleanup and existing-library preservation, duplicate-request rejection, stale progress rejection, first-frame loading and failure handling, derived-mask creation and repair, decoded-asset reuse, Dynamic Island and connected-notch masking, rounded exterior transparency, two-pixel bezel overlap, viewport-sized preview rendering, full-resolution output, stale frame-selection rejection, device matching, video orientation, codecs, audio, transparency, playback-position preservation, playback behavior, and target bitrate. The import benchmark runs only when `SIMFRAME_RUN_IMPORT_PERFORMANCE_TEST=1` so normal test runs do not repeatedly scan and encode the personal high-resolution fixture directory.
...
- On 2026-08-31, the final unit/media run executed all 36 tests with 1 opt-in import benchmark skipped and 0 failures in 11.747 seconds. Both local `Downloads`-backed iPhone 17 and transparent-MOV/audio regressions passed. A preceding full macOS command executed its then-current 35 unit/media tests with 1 skip and 0 failures before the onboarding UI test failed during runner startup because Xcode could not terminate stale reported SimFrame PID `78349`.
- On 2026-08-31, `./script/build_and_run.sh --verify` succeeded with the import-progress implementation. Running-app inspection replaced the real 30-frame library and confirmed the centered overlay, disabled workspace and toolbar, determinate progress at `0 of 30` and `19 of 30`, successful restoration after first-frame loading, and safe cancellation on a second import. A video import started from a nonzero timeline, paused with its controls visible beneath the overlay, and retained 11.62 seconds after cancellation.
